...
function layer:sample(imgs, opt)
  ...
  for t=1,self.seq_length+1 do
    ...
    local inputs = {x_word, imgs, unpack(state)}
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+1 do
    ...
    if not can_skip then
      ...
      self.inputs[t] = {x_word, imgs, unpack(self.state[t-1])}

      -- self.inputs[t]: {bz*128, bz*196*512, bz*128, bz*128}      
...
function layer:updateGradInput(input, gradOutput)
  ...
  for t=self.tmax,1,-1 do
    ...
    local dxt = {dinputs[1], dinputs[2]} -- first two element is the input vector
     
    dstate[t-1] = {} -- copy over rest to state grad
    -- staring from 3 to self.num_state+2 for attention model, starting from 2 to self.num_state + 1 for no attention(input the image at very timesetp) 
    for k=3,self.num_state+2 do table.insert(dstate[t-1], dinputs[k]) end 
   
    -- continue backprop of xt 
    local dwords_t, dimgs_t = unpack(dxt) 
