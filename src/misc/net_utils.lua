...
-- load glove vector 
function net_utils.load_glove(path, inputDim)
        ...
        for entry in line:gmatch("%S+") do -- split the line at each space
            if k == 1 then 
                ...
                if string.len(word) > 0 then 
                    ...
                    glove_table[word] = torch.zeros(inputDim, 1) -- padded with an extra dimention for convolution 
                else 
                    break 
                end 
            else
...
function net_utils.language_eval(predictions, id)
  ...
  utils.write_json('flickr8k-caption/val' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  os.execute('./misc/call_python_caption_eval.sh val' .. id .. '.json') -- i'm dying over here
  local result_struct = utils.read_json('flickr8k-caption/val' .. id .. '.json_out.json') -- god forgive me
