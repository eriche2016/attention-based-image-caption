...
-- VGG16 CNN model 
cmd:option('-cnn_proto','../../work_icpr2016/cnn_model/VGG_ILSVRC_16_layers_deploy.prototxt','path to CNN prototxt file in Caffe format. Note this MUST be a VGGNet-16 right now.')
cmd:option('-cnn_model','../../work_icpr2016/cnn_model/VGG_ILSVRC_16_layers.caffemodel','path to CNN model file containing the weights, Caffe format. Note this MUST be a VGGNet-16 right now.')
...
cmd:option('-finetune_cnn_after', 5000, 'After what iteration do we start finetuning the CNN? (-1 = disable; never finetune, 0 = finetune from start)')
...
cmd:option('-val_images_use', 4980, 'how many images to use when periodically evaluating the validation loss? (-1 = all)')
...
if isDebug then
  ...
  protos.cnn = net_utils.build_cnn_v2(cnn_raw, {encoding_size = opt.image_encoding_size, backend = cnn_backend})
  ...
  protos.expander = nn.Feat2DExpander(opt.seq_per_img)
